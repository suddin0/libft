...
int ambig_set_add(t_ambig_set *ambig_set, const t_option *options, int val, int match_type)
{
	int match;
	int i;
...
int longopt_err(t_getopt_internal_args arg, char *argv, const char *opt, int err)
{
	if(err == 1)
	{
		if(arg.print_error)
...
int manage_long_opt(t_getopt_internal_args arg, int *longindex, t_ambig_set *ambig_set, int argument)
{
	int index;
